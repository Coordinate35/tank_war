...
function form_validation.is_screen(data)
	local msg = "不合法"
	if nil == screen then 
		return false, msg
	end
	if nil == helper.atof(screen[1]) or nil == helper.atof(screen[2]) then
		return false, msg
	end
end

function form_validation.is_command(data)
	local msg = "不合法"
	local command = {"hello", "start", "move", "fire"}
...
function form_validation.check(params, check_content)
	local form_check_item_config = loadfile(ngx.var.root .. "/v1/config/form_check_item_config.lua")()
	local params_stack = {}
